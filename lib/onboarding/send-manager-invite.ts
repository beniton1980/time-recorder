...
export function createManagerInviteMail(mail: ManagerInviteMail) {
  ...
  return {
    ...
    html: `<p>${escapeHtml(mail.managerName)} 様</p>
        ...
        <p>※リンクは1回のみ利用可能です。<br>※有効期限：${escapeHtml(expiresAt)}<br>※お心当たりがない場合は、このメールを破棄してください。</p>`,
  };
}
...
export async function sendManagerInviteMail(
  ...
  const content = createManagerInviteMail(mail);
