...
export async function generateStorePosterPdf({ storeName, qrPngDataUrl }) {
  ...
  const qrSize = 205;
  doc.image(qrPngBuffer(qrPngDataUrl), centerX - qrSize / 2, 151, { width: qrSize, height: qrSize });
  doc.font("NotoSansJP-Regular").fontSize(10.5).fillColor("#333333")
    .text(`${storeName} の打刻用QRです。`, 102, 365, {
      width: contentWidth - 120,
      align: "center",
      lineGap: 3,
    });
  doc.font("NotoSansJP-Bold").fontSize(10).fillColor("#20334f")
    .text("打刻にはLINEアカウントが必要です。", 82, 407, {
  ...
  const stepsTop = 433;
  doc.roundedRect(boxX, stepsTop, boxWidth, 253, 10).fillAndStroke("#f7f7f3", "#d8d8d0");

  const steps = [
    ["1", "LINEの読み取り画面を開く", "「トーク」上部の「＋」→「QRコードスキャン」\n見当たらないときは「友だち追加」→「QRコード」。"],
    ["2", "このQRにカメラを向ける", "QR全体を枠に入れ、表示されたリンクを押します。"],
    ["3", "店舗名・名前を確認して打刻する", "初回だけ氏名を入力。出勤・休憩・退勤を選びます。"],
    ["4", "「記録しました」が出たら完了", "表示されるまで、この画面でお待ちください。"],
  ];
  ...
  const helpTop = 696;
  doc.roundedRect(boxX, helpTop, boxWidth, 74, 10).fillAndStroke("#edf4ee", "#c8d6cb");
  doc.font("NotoSansJP-Bold").fontSize(12).fillColor("#20334f")
    .text("打刻を間違えたとき・忘れたとき", boxX + 18, helpTop + 10, { width: boxWidth - 36 });
  doc.font("NotoSansJP-Regular").fontSize(10).fillColor("#303a33")
    .text("打刻画面の「打刻修正」から修正できます。\n分からない場合は、店舗の管理者に確認してください。", boxX + 18, helpTop + 32, {
      width: boxWidth - 36,
      lineGap: 4,
    });
