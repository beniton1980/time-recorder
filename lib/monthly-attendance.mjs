...
export function assessAttendance(events, pendingCorrections = []) {
  const days = new Map();
  const getDay = (staffId, businessDate, legalName) => {
    const key = `${staffId}:${businessDate}`;
    if (!days.has(key)) days.set(key, { staffId, businessDate, legalName, reasons: new Set(), gps: [] });
  ...
  for (const event of ordered) {
    const day = getDay(event.staff_id, event.business_date, event.legal_name);
  ...
  for (const correction of pendingCorrections) {
    getDay(correction.staff_id, correction.business_date, correction.legal_name).reasons.add("PENDING_CORRECTION");
  }
