...
const actions = new Set(['bootstrap', 'createPeriod', 'self', 'manager', 'saveDraft', 'submit', 'proxySubmit', 'closePeriod']);
...
export async function executeShiftRequest({ sql, lineIdentity, storeId, action, input }) {
  if (!/^[A-Za-z0-9_-]{1,255}$/.test(lineIdentity || '') || !uuid.test(storeId || '')
  ...
  const result = await sql.transaction(tx => [
    ...
    tx`SELECT set_config('shift.product','shift',true), set_config('shift.line_user_id',${lineIdentity},true), set_config('shift.store_id',${storeId.toLowerCase()},true)`,
    tx`SELECT shift.request(${action},${encoded}::jsonb) AS value`,
  ]);
