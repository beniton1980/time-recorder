...
function dayEventSummary(events) {
  const detailCheckIn = events.find((event) => event.label === "出勤")?.time ?? "";
  const detailCheckOut = events.findLast((event) => event.label === "退勤")?.time ?? "";
  const breakPeriods = [];
  let breakStart = null;
  ...
  return {
    checkIn: minuteTime(detailCheckIn),
    checkOut: minuteTime(detailCheckOut),
    detailCheckIn,
    detailCheckOut,
    breakPeriods,
  };
  ...
  return {
    ...
    staff: [...staff.entries()].map(([staffId, member]) => {
      ...
      for (const [businessDate] of member.byDate) {
        ...
        const reviewReasons = day.reviewReasons ?? [];
        if (reviewReasons.length > 0) reasons.push(`${businessDate}: ${reviewReasons.join(" / ")}`);
...
export function monthlyAttendanceIssues(report) {
  return report.staff.flatMap((staff) => staff.dailyAttendance
    .filter((day) => day.attendanceReasons.length > 0 || day.reviewReasons.length > 0)
    .map((day) => ({
      staffName: staff.name,
      businessDate: day.businessDate,
      reasons: [...day.attendanceReasons, ...day.reviewReasons],
    })));
}

export function monthlyAttendanceGpsIssues(report) {
  return report.staff.flatMap((staff) => staff.dailyAttendance
    .filter((day) => day.gpsIssueCount > 0)
    .map((day) => ({
      staffName: staff.name,
      businessDate: day.businessDate,
    })));
...
export function monthlyAttendanceStaffSummaries(report) {
  return report.staff.map((staff) => ({
    name: staff.name,
    workDays: staff.workDays,
    workMinutes: staff.workMinutes,
    lateNightMinutes: staff.lateNightMinutes,
  }));
}
