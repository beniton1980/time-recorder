const componentKeys = new Set([
  ...
  "statutoryHolidayPremium", "lateNightPremium", "adjustments",
]);
...
export function createPayrollSnapshotCsv({ storeName, run, items }) {
  const headers = [
    ...
    "月60時間超割増(円)", "法定休日割増(円)", "深夜割増(円)", "調整額(円)",
  ...
  for (const item of items) {
    ...
    lines.push([
      ...
      integer(components.lateNightPremium), integer(components.adjustments), integer(item.gross_pay_yen),
