...
function pageHeader(command, report, title, subtitle) {
  ...
  text(command, MARGIN, 26, `${report.storeName}  |  ONOGAMI蜍､諤`, 8, "0.40 0.47 0.43");
}

function summaryPage(report) {
  const command = [];
  pageHeader(command, report, `${report.storeName}  ${report.label}蜍､諤陦ｨ`, `蟇ｾ雎｡譛滄俣: ${report.period.start} - ${report.period.end}`);
  const cards = [
    ["繧ｹ繧ｿ繝・ヵ", `${report.staff.length}蜷港],
    ["蜍､諤隕∫｢ｺ隱・, `${report.staff.reduce((sum, staff) => sum + staff.attendanceIssueDays, 0)}譌･蛻・],
    ["GPS遒ｺ隱・, `${report.staff.reduce((sum, staff) => sum + staff.gpsIssueCount, 0)}莉ｶ`],
  ];
  ...
  let y = 638;
  text(command, MARGIN, y, "繧ｹ繧ｿ繝・ヵ蛻･繧ｵ繝槭Μ繝ｼ", 13);
  y -= 26;
  fill(command, MARGIN, y - 8, PAGE_WIDTH - MARGIN * 2, 26, "0.86 0.92 0.88");
  [[0, "繧ｹ繧ｿ繝・ヵ"], [180, "蜃ｺ蜍､譌･謨ｰ"], [262, "蜍､蜍呎凾髢・], [350, "莨第・譎る俣"], [438, "遒ｺ隱・]].forEach(([offset, label]) => text(command, MARGIN + Number(offset) + 7, y, label, 8));
  y -= 28;
  if (report.staff.length === 0) {
    text(command, MARGIN + 8, y - 12, "莉雁屓縺ｮ邱繧∵悄髢薙↓縺ｯ蜍､諤險倬鹸縺後≠繧翫∪縺帙ｓ縺ｧ縺励◆縲・, 11);
  } else {
    for (const staff of report.staff) {
      const status = staff.attendanceIssueDays > 0 ? "隕∫｢ｺ隱・ : "OK";
      text(command, MARGIN + 7, y, staff.name, 9);
      text(command, MARGIN + 187, y, `${staff.workDays}譌･`, 9);
  ...
  text(command, MARGIN, 53, `菴懈・譌･譎・ ${report.generatedAt}`, 8, "0.40 0.47 0.43");
  return command.join("\n");
}
...
function staffPage(report, staff) {
  ...
  text(command, MARGIN, 718, `蜃ｺ蜍､譌･謨ｰ  ${staff.workDays}譌･`, 10);
  text(command, 190, 718, `遒ｺ螳壽ｸ医∩蜍､蜍・ ${staff.workDuration}`, 10);
  text(command, 370, 718, `莨第・  ${staff.breakDuration}`, 10);
  text(command, MARGIN, 690, `蜍､諤隕∫｢ｺ隱・ ${staff.attendanceIssueDays}譌･蛻・, 10, staff.attendanceIssueDays ? "0.72 0.22 0.12" : "0.10 0.42 0.24");
  text(command, 260, 690, `GPS遒ｺ隱・ ${staff.gpsIssueCount}莉ｶ`, 10);
  let y = 650;
  fill(command, MARGIN, y - 8, PAGE_WIDTH - MARGIN * 2, 26, "0.86 0.92 0.88");
  [[0, "蝟ｶ讌ｭ譌･"], [92, "譎ょ綾"], [180, "謇灘綾"], [282, "迥ｶ諷・]].forEach(([offset, label]) => text(command, MARGIN + Number(offset) + 7, y, label, 8));
  y -= 28;
  for (const event of staff.events) {
    const marks = [event.corrected ? "險よｭ｣" : "", event.gpsIssue ? "窶ｻ菴咲ｽｮ遒ｺ隱・ : ""].filter(Boolean).join(" / ");
  ...
  if (staff.attendanceReasons?.length) {
    text(command, MARGIN, 82, `隕∫｢ｺ隱・ ${staff.attendanceReasons.join(" / ")}`, 8, "0.72 0.22 0.12");
  }
  return command.join("\n");
}
