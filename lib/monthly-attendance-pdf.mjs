...
const NOTO_REGULAR = fileURLToPath(new URL("../assets/fonts/NotoSansJP-Regular.otf", import.meta.url));
const NOTO_BOLD = fileURLToPath(new URL("../assets/fonts/NotoSansJP-Bold.otf", import.meta.url));
...
function drawLedger(command, x, topY, width, dates, byDate) {
  ...
  for (const businessDate of dates) {
    ...
    if (day) {
      ...
      const emptyWidth = columns.slice(1, 6).reduce((sum, value) => sum + value, 0);
      cellText(command, cursorX, y, emptyWidth, rowHeight, "-", 7.1, "#aaaaaa");
      cursorX += emptyWidth;
    }
...
function drawDetailRow(command, x, y, height, columns, day) {
  ...
  values.forEach((value, index) => {
    if (index === 2) cellLines(command, cursorX, bottom, columns[index], height, breakLines, 7.15, TEXT, false, "left", 4);
...
function detailPageBase(command, report, staff, detailPage, detailPageCount) {
  ...
  const columns = [74, 55, 190, 55, 68, 72, 55, PAGE_WIDTH - MARGIN * 2 - 569];
