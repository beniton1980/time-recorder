...
const SECTION_CONFIG: Record<string, SectionConfig> = {
  ...
  "給与集計": { key: "payroll", order: 5, collapsible: true },
  "QR・掲示物": { key: "qr", order: 6, collapsible: true },
  "共同管理者": { key: "co-managers", order: 7, collapsible: true },
  "店舗の打刻位置": { key: "store-location", order: 8, collapsible: true },
};

function referenceSection(shell: HTMLElement) {
  return Array.from(shell.children).find(
    (node): node is HTMLElement => node instanceof HTMLElement && node.tagName === "SECTION",
  ) ?? null;
}

function ensurePayrollSection(shell: HTMLElement) {
  if (shell.querySelector<HTMLElement>('[data-dashboard-key="payroll"]')) return;
  const reference = referenceSection(shell);
  if (!reference) return;
  ...
  link.href = PAYROLL_LIFF_URL;
  link.textContent = "給与集計を開く";
  link.setAttribute("aria-label", "LINE認証で給与集計画面を開く");
