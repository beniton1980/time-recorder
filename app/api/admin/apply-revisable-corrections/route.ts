...
export async function GET() {
  try {
    ...
    await sql`
      ...
      WITH latest_original_modifiers AS (
        SELECT DISTINCT ON (cr.target_event_id)
          cr.id, cr.target_event_id, cr.operation,
          cr.requested_event_type, cr.requested_occurred_at, cr.approved_at
        FROM correction_requests cr
        WHERE cr.status = 'APPROVED'
          AND cr.operation IN ('REPLACE', 'VOID')
          AND cr.target_event_id IS NOT NULL
        ORDER BY cr.target_event_id, cr.approved_at DESC NULLS LAST,
          cr.requested_at DESC, cr.id DESC
      ),
      latest_addition_modifiers AS (
        SELECT DISTINCT ON (cr.target_correction_id)
          cr.id, cr.target_correction_id, cr.operation,
          cr.requested_event_type, cr.requested_occurred_at, cr.approved_at
        FROM correction_requests cr
        WHERE cr.status = 'APPROVED'
          AND cr.operation IN ('REPLACE', 'VOID')
          AND cr.target_correction_id IS NOT NULL
        ORDER BY cr.target_correction_id, cr.approved_at DESC NULLS LAST,
          cr.requested_at DESC, cr.id DESC
      ),
      effective_originals AS (
        SELECT
          pe.id AS effective_id, pe.id AS original_event_id,
          lm.id AS correction_request_id, pe.store_id, pe.staff_id,
          ...
          CASE
            WHEN lm.operation = 'REPLACE' AND lm.requested_occurred_at IS NOT NULL
            THEN (((lm.requested_occurred_at AT TIME ZONE s.timezone)
              - make_interval(mins => s.business_day_start_minute)))::date
            ELSE pe.business_date
          END AS business_date,
          pe.latitude, pe.longitude, pe.gps_accuracy_m,
          pe.distance_from_store_m, pe.location_status,
          pe.validation_status, pe.validation_code, pe.source, pe.created_at,
          (lm.operation = 'REPLACE') AS corrected,
          NULL::uuid AS origin_correction_id
        FROM punch_events pe
        JOIN stores s ON s.id = pe.store_id
        LEFT JOIN latest_original_modifiers lm ON lm.target_event_id = pe.id
        WHERE lm.operation IS DISTINCT FROM 'VOID'
      ),
      effective_additions AS (
        SELECT
          addition.id AS effective_id, NULL::uuid AS original_event_id,
          COALESCE(lm.id, addition.id) AS correction_request_id,
          addition.store_id, addition.staff_id,
          ...
          (((COALESCE(lm.requested_occurred_at, addition.requested_occurred_at)
            AT TIME ZONE s.timezone)
            - make_interval(mins => s.business_day_start_minute)))::date AS business_date,
          ...
          CASE WHEN lm.operation = 'REPLACE'
            THEN 'APPROVED_CORRECTION_REPLACE'
            ELSE 'APPROVED_CORRECTION_ADD'
          END::text AS validation_code,
          'CORRECTION'::text AS source, addition.created_at,
          TRUE AS corrected, addition.id AS origin_correction_id
    ...
    const summary = await sql`
      SELECT
        COUNT(*)::int AS effective_events,
        COUNT(*) FILTER (WHERE corrected)::int AS corrected_events,
        COUNT(*) FILTER (WHERE origin_correction_id IS NOT NULL)::int AS added_events
    ...
    console.error("Revisable correction schema failed", error);
