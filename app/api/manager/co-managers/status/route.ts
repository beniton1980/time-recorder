...
export async function POST(request: Request) {
  ...
  const limited = await enforceRateLimit(request, { scope: "manager-status", limit: 30, windowSeconds: 300 }, body.idToken);
  if (limited) return limited;
  try {
    const identity = await verifyLineIdToken(body.idToken);
    const sql = getSql({ mode: "manager", lineIdentity: identity.sub, storeId: body.storeId });
    const rows = await sql`SELECT * FROM set_store_manager_status(${identity.sub}, ${body.storeId}::uuid, ${body.staffId}::uuid, ${String(body.status)})`;
    if (rows.length === 0) return NextResponse.json({ ok: false, code: "STATUS_CHANGE_NOT_ALLOWED" }, { status: 409 });
    ...
    logServerError("manager_status_change_failed");
