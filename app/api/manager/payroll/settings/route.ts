...
type InitialCompensationInput = { staffId: string; hourlyRateYen: number; effectiveFrom: string };
type CompensationTermRow = { id: string; staff_id: string; hourly_rate_yen: number; effective_from: string; effective_to: string | null; created_at?: string };
...
function validCompensationInput(body: RequestBody): body is ValidCompensationBody {
  return typeof body.staffId === "string" && uuidPattern.test(body.staffId) && typeof body.hourlyRateYen === "number" && Number.isInteger(body.hourlyRateYen) && body.hourlyRateYen > 0 && validDate(body.effectiveFrom);
}
function parseInitialCompensationTerms(value: unknown): InitialCompensationInput[] | null {
  if (!Array.isArray(value) || value.length < 1 || value.length > 100) return null;
  const terms: InitialCompensationInput[] = []; const staffIds = new Set<string>();
  for (const item of value) {
    if (typeof item !== "object" || item === null) return null;
    const candidate = item as Record<string, unknown>;
    if (typeof candidate.staffId !== "string" || !uuidPattern.test(candidate.staffId) || staffIds.has(candidate.staffId)) return null;
    if (typeof candidate.hourlyRateYen !== "number" || !Number.isInteger(candidate.hourlyRateYen) || candidate.hourlyRateYen <= 0 || !validDate(candidate.effectiveFrom)) return null;
...
function validStoreSettings(body: RequestBody) {
  ...
  if (body.statutoryHolidayRule === "FIXED_WEEKDAY") return typeof body.statutoryHolidayWeekday === "number" && Number.isInteger(body.statutoryHolidayWeekday) && body.statutoryHolidayWeekday >= 0 && body.statutoryHolidayWeekday <= 6;
...
async function authenticate(request: Request, body: RequestBody) {
  if (typeof body.idToken !== "string" || typeof body.storeId !== "string" || !uuidPattern.test(body.storeId)) return { error: NextResponse.json({ ok: false, code: "INVALID_INPUT" }, { status: 400 }) } as const;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "saveStoreSettings") {
      if (!validStoreSettings(body)) return NextResponse.json({ ok: false, code: "INVALID_PAYROLL_STORE_SETTINGS" }, { status: 400 });
      const weekday = body.statutoryHolidayRule === "FIXED_WEEKDAY" ? body.statutoryHolidayWeekday : null;
      const rows = await sql`INSERT INTO payroll_store_settings (store_id, work_time_system, overtime_month_rule, statutory_holiday_rule, statutory_holiday_weekday, updated_at) VALUES (${storeId}::uuid, ${body.workTimeSystem}, ${body.overtimeMonthRule}, ${body.statutoryHolidayRule}, ${weekday}, NOW()) ON CONFLICT (store_id) DO UPDATE SET work_time_system = EXCLUDED.work_time_system, overtime_month_rule = EXCLUDED.overtime_month_rule, statutory_holiday_rule = EXCLUDED.statutory_holiday_rule, statutory_holiday_weekday = EXCLUDED.statutory_holiday_weekday, updated_at = NOW() RETURNING store_id, work_time_system, week_starts_on, overtime_month_rule, statutory_holiday_rule, statutory_holiday_weekday, overtime_premium_rate::float8 AS overtime_premium_rate, high_overtime_premium_rate::float8 AS high_overtime_premium_rate, statutory_holiday_premium_rate::float8 AS statutory_holiday_premium_rate, late_night_premium_rate::float8 AS late_night_premium_rate, rounding_mode`;
      return NextResponse.json({ ok: true, settings: rows[0] });
    }

    if (action === "saveStatutoryHolidayMonth") {
      if (!validMonth(body.holidayMonth) || !Array.isArray(body.holidayDates) || body.holidayDates.length > 31) return NextResponse.json({ ok: false, code: "INVALID_STATUTORY_HOLIDAY_DATE" }, { status: 400 });
      const holidayDates = [...new Set(body.holidayDates)].sort();
      if (!holidayDates.every((date) => validDate(date) && date.startsWith(`${body.holidayMonth}-`))) return NextResponse.json({ ok: false, code: "INVALID_STATUTORY_HOLIDAY_DATE" }, { status: 400 });
    ...
    if (action === "addStatutoryHolidayDate") {
      if (!validDate(body.holidayDate)) return NextResponse.json({ ok: false, code: "INVALID_STATUTORY_HOLIDAY_DATE" }, { status: 400 });
      await sql`INSERT INTO payroll_statutory_holidays (store_id, holiday_date, created_by_line_user_id) VALUES (${storeId}::uuid, ${body.holidayDate}::date, ${identity.sub}) ON CONFLICT (store_id, holiday_date) DO NOTHING`;
      return NextResponse.json({ ok: true });
    }
    if (action === "removeStatutoryHolidayDate") {
      if (!validDate(body.holidayDate)) return NextResponse.json({ ok: false, code: "INVALID_STATUTORY_HOLIDAY_DATE" }, { status: 400 });
      await sql`DELETE FROM payroll_statutory_holidays WHERE store_id = ${storeId}::uuid AND holiday_date = ${body.holidayDate}::date`;
      return NextResponse.json({ ok: true });
    }

    if (action === "saveInitialCompensationTerms") {
      const terms = parseInitialCompensationTerms(body.initialCompensationTerms);
      if (!terms) return NextResponse.json({ ok: false, code: "INVALID_COMPENSATION_TERM" }, { status: 400 });
      const staffIds = terms.map((term) => term.staffId);
      const existing = await sql`SELECT staff_id FROM payroll_compensation_terms WHERE store_id = ${storeId}::uuid AND staff_id = ANY(${staffIds}::uuid[]) LIMIT 1`;
      if (existing.length > 0) return NextResponse.json({ ok: false, code: "COMPENSATION_HISTORY_EXISTS" }, { status: 409 });
      try {
        const inserted = await sql.transaction((tx) => terms.map((term) => tx`INSERT INTO payroll_compensation_terms (store_id, staff_id, hourly_rate_yen, effective_from, created_by_line_user_id) VALUES (${storeId}::uuid, ${term.staffId}::uuid, ${term.hourlyRateYen}, ${term.effectiveFrom}::date, ${identity.sub}) RETURNING id, staff_id, hourly_rate_yen, effective_from::text, effective_to::text, created_at`));
        return NextResponse.json({ ok: true, compensationTerms: inserted.flat() });
      } catch (error) {
        if (postgresCode(error) === "23P01") return NextResponse.json({ ok: false, code: "COMPENSATION_HISTORY_EXISTS" }, { status: 409 });
        throw error;
      }
    }

    if (action === "createInitialCompensationTerm") {
      if (!validCompensationInput(body)) return NextResponse.json({ ok: false, code: "INVALID_COMPENSATION_TERM" }, { status: 400 });
      const existing = await sql`SELECT 1 FROM payroll_compensation_terms WHERE store_id = ${storeId}::uuid AND staff_id = ${body.staffId}::uuid LIMIT 1`;
      if (existing.length > 0) return NextResponse.json({ ok: false, code: "COMPENSATION_HISTORY_EXISTS" }, { status: 409 });
      const rows = await sql`INSERT INTO payroll_compensation_terms (store_id, staff_id, hourly_rate_yen, effective_from, created_by_line_user_id) VALUES (${storeId}::uuid, ${body.staffId}::uuid, ${body.hourlyRateYen}, ${body.effectiveFrom}::date, ${identity.sub}) RETURNING id, staff_id, hourly_rate_yen, effective_from::text, effective_to::text, created_at`;
      return NextResponse.json({ ok: true, compensationTerm: rows[0] });
    }

    if (action === "reviseCompensationTerm") {
      if (!validCompensationInput(body)) return NextResponse.json({ ok: false, code: "INVALID_COMPENSATION_TERM" }, { status: 400 });
      const openRows = await sql`SELECT id, staff_id, hourly_rate_yen, effective_from::text, effective_to::text FROM payroll_compensation_terms WHERE store_id = ${storeId}::uuid AND staff_id = ${body.staffId}::uuid AND effective_to IS NULL ORDER BY effective_from DESC` as CompensationTermRow[];
      if (openRows.length !== 1) return NextResponse.json({ ok: false, code: "COMPENSATION_CURRENT_TERM_REQUIRED" }, { status: 409 });
      const current = openRows[0];
      if (body.effectiveFrom <= current.effective_from) return NextResponse.json({ ok: false, code: "COMPENSATION_REVISION_DATE_INVALID" }, { status: 409 });
      try {
        const [, insertedRows] = await sql.transaction((tx) => [
          tx`UPDATE payroll_compensation_terms SET effective_to = ${body.effectiveFrom}::date - 1 WHERE id = ${current.id}::uuid AND store_id = ${storeId}::uuid AND staff_id = ${body.staffId}::uuid AND effective_to IS NULL`,
          tx`INSERT INTO payroll_compensation_terms (store_id, staff_id, hourly_rate_yen, effective_from, created_by_line_user_id) VALUES (${storeId}::uuid, ${body.staffId}::uuid, ${body.hourlyRateYen}, ${body.effectiveFrom}::date, ${identity.sub}) RETURNING id, staff_id, hourly_rate_yen, effective_from::text, effective_to::text, created_at`,
        ]);
        return NextResponse.json({ ok: true, compensationTerm: insertedRows[0] });
      } catch (error) {
        if (postgresCode(error) === "23P01") return NextResponse.json({ ok: false, code: "COMPENSATION_PERIOD_OVERLAP" }, { status: 409 });
        throw error;
      }
    }
